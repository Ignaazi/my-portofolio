    ...
    >
      ...
      <button 
        ...
        className={`flex items-center gap-2 px-5 py-2 rounded-full font-bold text-xs uppercase tracking-widest transition-all mb-8 border ${
          isDarkMode 
          ? "bg-slate-800 border-slate-700 text-white hover:bg-orange-600 hover:border-orange-600" 
          : "bg-white border-slate-200 text-slate-900 hover:bg-orange-600 hover:text-white hover:border-orange-600 shadow-sm"
        }`}
      >
      ...
      <header className="mb-8">
        ...
        <div className={`flex flex-wrap items-center gap-4 text-xs font-bold uppercase tracking-wider opacity-60 pb-6 border-b border-dashed ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
          ...
          <span>•</span>
          <span className="flex items-center gap-1.5"><Clock size={14} className="text-orange-600"/> {post.date}</span>
        </div>
      </header>
      ...
      <article className={`space-y-6 text-[17px] leading-relaxed mb-12 ${
        ...
        <div className="pt-8 border-t border-slate-500/10">
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag, i) => (
              <span key={i} className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-[11px] font-black uppercase transition-all ${
                isDarkMode ? tag.dark : tag.bg
              }`}>
      ...
      <footer className="mt-20 pt-8 border-t border-slate-500/10 flex justify-between items-center opacity-30">
        <span className="text-[10px] font-black uppercase tracking-[0.3em]">Ajitech System</span>
        <span className="text-[10px] font-black uppercase tracking-[0.3em]">2026</span>
      </footer>
    </motion.div>
  );
};
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              <div className="lg:col-span-8 space-y-8" style={{ perspective: "1000px" }}>
                    ...
                    <article className={`flex flex-col md:flex-row gap-6 p-6 rounded-[2.5rem] border-2 transition-all duration-500 ${
                      ...
                      <div className="flex-1 flex flex-col justify-between py-1">
                        <div>
                          ...
                          <h3 onClick={() => setSelectedPost(post)} className={`text-2xl font-black italic uppercase leading-tight mb-4 cursor-pointer hover:text-orange-600 transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                              {post.title}
                          </h3>
                        </div>
                        <div className="flex gap-6 pt-4 border-t border-orange-500/20">
                          <button onClick={() => handleLikePost(post.id)} className="flex items-center gap-2 text-xs font-black transition-all hover:text-orange-600 group">
                              <Heart size={18} className={likes.some(l => Number(l.post_id) === post.id && l.user_name === (userName || "Anonymous_Liker")) ? "fill-orange-600 text-orange-600" : "text-slate-400 group-hover:text-orange-600"} /> 
                ...
                {totalPages > 1 && (
                  <div className="flex justify-center items-center gap-4 pt-10">
                      <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className="p-3 rounded-xl border-2 border-orange-600/20 hover:border-orange-600 disabled:opacity-20 transition-all"><ChevronLeft size={20} /></button>
                      <div className="flex gap-2">
                          {[...Array(totalPages)].map((_, i) => (
                              <button key={i} onClick={() => setCurrentPage(i + 1)} className={`w-10 h-10 rounded-xl font-black text-xs transition-all ${currentPage === i + 1 ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/30' : 'bg-orange-600/10 opacity-40 hover:opacity-100'}`}>{i + 1}</button>
                          ))}
                      </div>
                      <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)} className="p-3 rounded-xl border-2 border-orange-600/20 hover:border-orange-600 disabled:opacity-20 transition-all"><ChevronRight size={20} /></button>
                  </div>
                )}
              </div>
              ...
              <div className="lg:col-span-4 space-y-8">
                <div className="lg:sticky lg:top-10 space-y-8">
                  <div className={`rounded-[2.5rem] p-6 border-2 transition-all duration-500 ${isDarkMode ? "bg-slate-900 border-white/10 shadow-2xl" : "bg-white border-slate-200 shadow-xl"}`}>
                     <h3 className={`text-[10px] font-black uppercase tracking-[0.4em] mb-8 flex items-center justify-between opacity-80 ${isDarkMode ? 'text-white' : 'text-slate-600'}`}>Ranking System <TrendingUp size={16} className="text-orange-600 animate-pulse" /></h3>
                     <div className="space-y-4" style={{ perspective: "1500px" }}>
                        <AnimatePresence mode="popLayout">
                          {leaderboard.map((user, i) => {
                             const isRank1 = i === 0; const isRank2 = i === 1; const isRank3 = i === 2; const isRank4 = i === 3;
                             let cardStyle = ""; let zHeight = 0;
                                 ...
                                 <div className="flex items-center gap-4 relative z-10" style={{ transform: "translateZ(25px)" }}>
                                    ...
                                    <div className="flex flex-col">
                                      ...
                                      <span className={`text-[7px] font-bold tracking-widest uppercase ${i < 3 ? 'text-white/60' : 'opacity-40'}`}>
                                        {isRank1 ? "Supreme" : isRank2 ? "Legend" : i === 2 ? "Master" : i === 3 ? "Expert" : "Challenger"}
                                      </span>
                                    </div>
                                 </div>
                  ...
                  <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className={`rounded-[2rem] p-4 border-2 transition-all duration-500 mt-6 relative overflow-hidden ${isDarkMode ? "bg-slate-950 border-white/10 shadow-2xl" : "bg-white border-slate-200 shadow-xl"}`}>
                      <h3 className={`text-[9px] font-black uppercase tracking-[0.4em] mb-4 flex items-center justify-between opacity-80 ${isDarkMode ? 'text-white' : 'text-slate-600'}`}>Support Node <Wallet size={14} className="text-orange-600 animate-bounce" /></h3>
                      <div className="space-y-3" style={{ perspective: "1000px" }}>
                          {[
                              ...
                              <motion.button key={idx} whileHover={{ scale: 1.02, rotateX: 10, rotateY: -5, translateZ: 25 }} onClick={() => { navigator.clipboard.writeText(item.account); alert(`${item.name} copied!`); }} className="relative w-full p-[1.5px] rounded-xl overflow-hidden group transition-all duration-500" style={{ transformStyle: "preserve-3d" }}>
                                  <motion.div animate={{ rotate: [0, 360] }} transition={{ repeat: Infinity, duration: 4, ease: "linear" }} className="absolute inset-[-100%] z-0" style={{ background: item.flow }} />
                                  <div className={`relative z-10 w-full flex items-center justify-between p-3.5 rounded-xl ${isDarkMode ? 'bg-slate-900/95' : 'bg-white'} transition-colors group-hover:bg-opacity-90`}>
                                      <div className="flex items-center gap-4" style={{ transform: "translateZ(15px)" }}>
                                          <div className={`w-11 h-7 rounded-lg bg-gradient-to-br ${item.color} flex items-center justify-center shadow-[0_0_15px_rgba(0,0,0,0.3)] border border-white/20 transform -rotate-6 group-hover:rotate-0 transition-all duration-500`}><span className="text-[9px] font-black text-white italic drop-shadow-md">{item.name}</span></div>
                                          <div className="text-left"><p className={`text-[10px] font-black italic tracking-wider leading-none ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{item.name} </p><div className="flex items-center gap-1 mt-1"><span className={`w-1 h-1 rounded-full animate-ping ${item.theme === 'fire' ? 'bg-orange-500' : 'bg-cyan-400'}`} /><p className="text-[6px] font-bold uppercase opacity-40 tracking-[0.1em]">Active</p></div></div>
                                      </div>
                                      <div className="relative" style={{ transform: "translateZ(30px)" }}><div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 border-dashed border-orange-600/30 group-hover:border-solid group-hover:bg-orange-600 transition-all duration-300`}><Copy size={12} className="text-orange-600 group-hover:text-white" /></div></div>
                                  </div>
                              </motion.button>
                          ))}
                      </div>
                      <div className="mt-1 pt-3 border-t border-white/5 text-center"><p className="text-[8px] font-black uppercase tracking-[0.2em] ">setiap dukungan untuk pembangunan website</p></div>
                  </motion.div>
                </div>
              </div>
            </div>
